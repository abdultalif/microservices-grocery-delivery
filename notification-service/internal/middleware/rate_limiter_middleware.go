	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if err != nil {
				...
				return c.JSON(http.StatusInternalServerError, response.ResponseDefaultError(http.StatusInternalServerError, "internal rate limiter error"))
			}
			...
			if val > int64(limit) {
				...
				return c.JSON(http.StatusTooManyRequests, response.ResponseDefaultError(http.StatusTooManyRequests, "too many requests"))
			}
