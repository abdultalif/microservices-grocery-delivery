...
type Product struct {
	...
	CategorySlug string         `gorm:"index;unique;column:category_slug;not null"`
	Name         string         `gorm:"column:name;not null"`
	Image        string         `gorm:"column:image;not null"`
	Description  string         `gorm:"column:description"`
	RegulerPrice float64        `gorm:"column:reguler_price;default:0"`
	SalePrice    float64        `gorm:"index;column:sale_price;default:0"`
	...
	Status       string         `gorm:"index;column:status;default:'DRAFT';size:20"`
